...
export default async function (hre: HardhatRuntimeEnvironment) {
  console.log(`Running deploy script for the TokenActionsModule contract`);
  ...
  const tokenActionsModuleContract = await deployer.deploy(artifact, [LOGGER_ADDRESS]);

  // Show the contract info.
  const contractAddress = tokenActionsModuleContract.address;
  console.log(`${artifact.contractName} was deployed to ${contractAddress}`);

  // verify contract for tesnet & mainnet
  if (process.env.NODE_ENV != "test") {
    // Contract MUST be fully qualified name (e.g. path/sourceName:contractName)
    const contractFullyQualifedName = "contracts/modules/TokenActionsModule.sol:TokenActionsModule";
